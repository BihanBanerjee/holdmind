...
from schemas.memory import ClaimDetailResponse, ClaimPatchRequest, GraphResponse
from services.memory_service import get_claim_detail, get_graph_data, patch_claim
...
def patch(
    request: Request,
    claim_id: str,
    body: ClaimPatchRequest,
    current_user: User = Depends(get_current_user),
    api_key: str = Depends(require_api_key),
):
    store = get_user_store(current_user.id, api_key)
    try:
        result = patch_claim(store, claim_id, body.label)
        if result is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Claim not found")
        return result
    finally:
